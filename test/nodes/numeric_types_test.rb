...
class NumericTypesTest < ActiveSupport::TestCase
  def setup
    @node_data = {
      name: 'Numeric Test Node',
      integer_value: 42,
      float_value: 3.14159,
      decimal_value: BigDecimal('123.456')
    }
  end
  ...
  test 'can create node with decimal attributes' do
    node = NumericTestNode.create!(@node_data)

    assert_equal BigDecimal('123.456'), node.decimal_value
    assert_instance_of BigDecimal, node.decimal_value
  end
  ...
  test 'can persist and retrieve numeric values' do
    ...
    assert_equal BigDecimal('123.456'), retrieved_node.decimal_value

    # Verify types are correct
    assert_instance_of Integer, retrieved_node.integer_value
    assert_instance_of Float, retrieved_node.float_value
    assert_instance_of BigDecimal, retrieved_node.decimal_value
  end
  ...
  test 'handles edge cases for numeric values' do
    ...
    assert_equal BigDecimal('0.0'), node.decimal_value
  end
  ...
  test 'can update numeric values' do
    ...
    node.update(
      integer_value: 100,
      float_value: 2.718,
      decimal_value: BigDecimal('456.789')
    )
    ...
    assert_equal BigDecimal('456.789'), updated_node.decimal_value
  end
